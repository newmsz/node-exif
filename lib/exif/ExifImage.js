...
function ExifImage (options, callback) {
  ...
  this._stringFormRationalNumber = options._stringFormRationalNumber || false;
  this._convertFields = options._convertFields || false;
  ...
  if (!options.image) {
    ...
    callback = function (err, res) {
      ...
      else if(res) self.emit('exif', res);
    };
  ...
  setImmediate(function () {
    self.loadImage(options.image, function (error, image) {
      if (error)
        callback(error);
      else
...
require('util').inherits(ExifImage, EventEmitter);
...
ExifImage.prototype.extractExifEntry = function (data, entryOffset, tiffOffset, isBigEndian, tags) {
  ...
  if(entry.components < 4292542590 && entry.valueOffset < 1000000) { // naive bug fix
    switch (entry.format) {
      ...
      case 0x0005: // unsigned rational, 8 byte per component (4 byte numerator and 4 byte denominator)
        ...
        for (var i = 0; i < entry.components; i++) {
          if(self._stringFormRationalNumber) entry.value.push(data.getLong(entry.valueOffset + i * 8, isBigEndian) + '/' + data.getLong(entry.valueOffset + i * 8 + 4, isBigEndian));
          else entry.value.push(data.getLong(entry.valueOffset + i * 8, isBigEndian) / data.getLong(entry.valueOffset + i * 8 + 4, isBigEndian));
        }
        break;
      ...
      case 0x000A: // signed rational, 8 byte per component (4 byte numerator and 4 byte denominator)
        entry.valueOffset = data.getLong(entryOffset + 8, isBigEndian) + tiffOffset;
        for (var i = 0; i < entry.components; i++) {
          if(self._stringFormRationalNumber) entry.value.push(data.getSignedLong(entry.valueOffset + i * 8, isBigEndian) + '/' + data.getSignedLong(entry.valueOffset + i * 8 + 4, isBigEndian));
          else entry.value.push(data.getSignedLong(entry.valueOffset + i * 8, isBigEndian) / data.getSignedLong(entry.valueOffset + i * 8 + 4, isBigEndian));
        }
        break;
  ...
  if(self._convertFields && ExifFieldConverter[entry.tagName]) entry.value = ExifFieldConverter[entry.tagName](entry.value);

  return entry;

};

ExifImage.prototype.getGPSTimeStamp = function () {
  if(!this.exifData || !this.exifData.gps || !this.exifData.gps.GPSTimeStamp) return null;
  return { hh: StringToRational(this.exifData.gps.GPSTimeStamp[0]), mm: StringToRational(this.exifData.gps.GPSTimeStamp[1]), ss: StringToRational(this.exifData.gps.GPSTimeStamp[2]) };
};
/**
...
var ExifFieldConverter = {
  ModifyDate:ExifDate, CreateDate:ExifDate, DateTimeOriginal:ExifDate,
  GPSLatitude: GPSArrayJoin, GPSLongitude: GPSArrayJoin, GPSTimeStamp: GPSArrayJoin
};

function ExifDate(val) {
  // YYYY:MM:DD HH:MM:SS -> ISOFormat
  var split = val.split(/ /);
  return new Date(split[0].replace(/:/g, '-') + 'T' + split[1] + 'Z').toISOString();
}

function GPSArrayJoin(arr) {
  return arr.join(' ');
}

function StringToRational(val) {
  var split = val.split(/\//);
